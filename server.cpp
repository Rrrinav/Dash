...
void add_to_epoll(int epfd, int fd, uint32_t events, void *ptr)
{
  epoll_event ev{};
  ev.events = events;
  ev.data.ptr = ptr;
...
struct Async_task
{
  ...
  bool done() const { return handle == nullptr || handle.done(); }
};
...
Async_task send_handler(Client *client)
{
...
void Client::try_start_next_send()
{
  ...
  else
  {
    ...
    static std::vector<Async_task> send_tasks;
    send_tasks.emplace_back(send_handler(this));
  }
}
...
void process_command(Client *client, const std::string &data)
{
  ...
  switch (cmd->_type)
  {
    ...
    {
      auto n = client->tree.insert(cmd->_path);
...
Async_task client_read(Client *client)
{
  ...
  // Clean up the client when exiting
  if (client)
...
Async_task accept_clients(int epfd, int listen_fd)
{
  ...
  while (true)
  {
    ...
    // Start client read coroutine - it will clean up itself when done
    static std::vector<Async_task> client_tasks;
    client_tasks.emplace_back(client_read(client));
  }
...
int main()
{
  int listen_fd = init_server(PORT, HOST);
